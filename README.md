...
| DMGR1 / DMGR2 | El swarm que sirve `dev`/`sit`/`qa` de verdad corre en un solo nodo (esta máquina). **Verificado por separado (2026-08-24)** que el patrón sí escala a multi-nodo real: se levantó un swarm aislado de prueba con dos contenedores Docker-in-Docker (`docker:dind`), cada uno con su propio `dockerd`, unidos como dos managers reales (`docker node ls` mostró `dmgr1` Leader + `dmgr2` Reachable), y se desplegó el `stack/base.yml` + `stack/qa.yml` de este mismo repo sin cambios — Swarm distribuyó las 2 réplicas una en cada nodo, y la app respondió `{"status":"ok"}` real dentro del contenedor. **Intento real con una segunda máquina física (2026-08-28):** con una segunda PC Windows disponible en la misma red, se intentó unirla de verdad al swarm que sirve `dev`/`sit`/`qa` — reveló una limitación arquitectónica real de **Docker Desktop para Windows**: el daemon corre dentro de una VM interna (WSL2/Hyper-V) con una red NAT propia (`192.168.65.3`), sin acceso a la IP LAN real de la máquina (`192.168.18.11`). Se probaron dos soluciones y ambas fallaron con el mismo error de fondo (`cannot assign requested address`): (1) forzar `--advertise-addr`/`--listen-addr` a la IP real, y (2) activar el modo "mirrored networking" de WSL2 (`.wslconfig`, requiere reiniciar Docker Desktop) — Docker Desktop gestiona su distro interna de forma especial y no respeta esa configuración. Ambos intentos se revirtieron limpiamente, con `dev`/`sit`/`qa` y `monitoring` restaurados y verificados sanos después de cada uno (dos interrupciones breves, ambas recuperadas con `stack/deploy.sh`). La solución real requeriría reemplazar Docker Desktop por Docker Engine nativo (Linux, o una VM con red *Bridged* vía VirtualBox/VMware en vez de la red NAT forzada de Docker Desktop) — se evaluó el alcance (migrar dónde corre todo el swarm, no solo unir un nodo) y se decidió no hacerlo por ser un reto personal, no una exigencia del proceso. En Interbank, la infraestructura real sería Linux nativo (servidores o VMs Linux), donde esta limitación no existe — es específica de usar Docker Desktop sobre Windows como entorno de práctica. |

...
Este repositorio **no busca demostrar una aplicación** — el código funcional es intencionalmente trivial (un "hola mundo" en FastAPI con dos endpoints: `/health` y `/hello`). El objetivo es reproducir, a escala reducida pero con herramientas reales, el flujo de CI/CD que se describió en el proceso de selección para el puesto de Desarrollador DevOps en Cinte Perú / Interbank (ver `reto.png`), de modo que cada pieza del pipeline (compilación, pruebas, análisis de seguridad, gates de calidad, aprobación manual, despliegue en Docker Swarm por ambientes) se entienda de primera mano antes de encontrarla en el entorno real de Interbank.
...
2. **Cadena de suministro de software (supply chain) cuidada**: las dependencias (Python y GitHub Actions) están vigiladas por Dependabot con alertas de vulnerabilidad automáticas, y las propias GitHub Actions usadas en los workflows están pineadas a un commit SHA exacto (no a un tag mutable), para que un mantenedor externo no pueda inyectar código simplemente re-apuntando una versión.
...
```
1. Un desarrollador hace push (o merge) a `main`.
2. GitHub Actions (ci.yml) corre en paralelo/secuencia:
...
3. Si build-test + quality-gate-sonar + fluid-attacks-equivalent pasan, y el push
   fue directo a main, publish-ghcr construye la imagen Docker y la sube a
   ghcr.io con dos tags: el SHA del commit y `latest`.
...
7. Cada job aprobado hace docker pull de la imagen desde ghcr.io y ejecuta
...
- El Swarm es de un solo nodo simulando DMGR1/DMGR2; en Interbank habría nodos físicos/VMs separados.

...
| "¿Por qué un solo nodo Swarm y no DMGR1/DMGR2 reales?" | Es una simulación en una sola máquina en el swarm que sirve `dev`/`sit`/`qa`, pero el patrón multi-nodo se verificó de verdad por separado: un swarm de prueba aislado con dos contenedores `docker:dind` unidos como managers reales, desplegando el `stack/base.yml` sin cambios y con Swarm repartiendo réplicas entre ambos nodos. Además se intentó con una segunda máquina física real, lo que reveló que Docker Desktop para Windows no puede exponer su daemon en la IP LAN real (vive en una VM aislada) — dos soluciones probadas y descartadas con evidencia (ver README). No es una suposición; es una limitación diagnosticada de la herramienta de desarrollo, no del concepto. |
